...
#Skewness-optimized portlolio

symbols = ['BULLC', 'ETHBULLC']
numofasset = len(symbols)

numofportfolio = 5000

df = pd.read_csv('C:/Users/Ronnawat/Desktop/Trading with Python/ลงทุนเป็นระบบ Finlab/Portfolio Optimization Constrained/FTX_DEMOPORT.csv', index_col=0, parse_dates=True)
#xw.view(df)
...
'''Optimized Portfolio 
In this case: 
Optimal weight for portfolio
BULLUSDT = 72.45%
ETHBULLUSDT = 27.55%

>> Optimal value from Meam Variace analysis
returns["OPVAL"] = returns["BULLC"]*0.7245 + returns["ETHBULLC"]*0.2755
skew(returns["OPVAL"])
kurtosis(returns["OPVAL"])
