...
# Define a function to compute ERC weights using ES-based Risk Contribution
def compute_erc_weights_es(cov_matrix, mean_returns, confidence_level=0.95):
    ...
    def risk_contribution(weights):
        ...
        rc = weights * (
            mean_returns + marginal_contrib / ((1 - confidence_level) * np.sqrt(portfolio_variance)) * z_score
        )
        return rc
...
confidence_level = 0.95  # Confidence level for ES
...
for start in range(0, len(returns) - window_size, rebalance_freq):
    ...
    # Calculate ERC weights based on ES Risk Contribution
    try:
        weights = compute_erc_weights_es(cov_matrix.values, mean_returns, confidence_level=confidence_level)
...
if len(portfolio_returns) == 0:
    ...
    plt.plot(cumulative_returns, label='Portfolio (ERC with ES RC)')
